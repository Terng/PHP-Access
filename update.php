<?php
    ...
    //$id = $_GET['id'];
    $selsql = " SELECT * FROM `tblEmployees` 
                WHERE pkeyEmployeeID=?";
    //$result = $db->query($sql);
    $selresult = $conn->prepare($selsql);
    $selres = $selresult->execute(array($_GET['pkeyEmployeeID']));
    ...
    if(isset($_POST) & !empty($_POST)){
        ...
        $sql = "UPDATE tblEmployees SET strFirstName=:firstname, strLastName=:lastname, strTitle=:strTitle, strTitleOfCourtesy=:Courtesy, dtmBirthDate=:dob WHERE pkeyEmployeeID=:id";
